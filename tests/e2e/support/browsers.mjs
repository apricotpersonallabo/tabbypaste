...
import { Builder, logging, until } from 'selenium-webdriver';
...
const prepareBrowserWindow = async (driver, extensionOrigin) => {
  ...
  await driver.wait(until.titleContains('Tabby Paste'), 10_000);
...
export const createChromiumBrowser = async () => {
  ...
  return {
    driver,
    extensionOrigin,
    shortcutUrl: process.env.CHROMIUM_SHORTCUT_URL || 'http://chromium:9010/shortcut'
  };
};
...
export const createFirefoxBrowser = async () => {
  ...
  return {
    driver,
    extensionOrigin,
    shortcutUrl: process.env.FIREFOX_SHORTCUT_URL || 'http://firefox:9010/shortcut'
  };
};
